...
// Function to generate a random expression with '+' and '-' operations
void generate_expression(char *buf, size_t size)
{
    int num_terms = rand() % 5 + 1; // between 1 and 5 segments 
    int value = rand() % 100;
    size_t len = snprintf(buf, size, "%d", value);
    const char ops[] = "+-";
    for (int i = 1; i < num_terms; ++i)
    {
        char op = ops[rand() % 2];
        value = rand() % 100;
        len += snprintf(buf + len, size - len, " %c %d", op, value);
    }
...
int main( int t_narg, char **t_args )
{
    ...
    while ( 1 )
    {
        ...
        // select from fds
        int poll_res = poll( l_read_poll, 2, 15000);
        ...
        {
            // User has not inputed any data for 15s, generate and send an expression
            generate_expression(l_buf, sizeof(l_buf));
            log_msg(LOG_INFO, "No input for 15 seconds. Generated expression: %s", l_buf);
